...
void polynomPrint(struct Polynom a) {
    ...
    for (int i = MAX_LEN-1; i >= 0; i--) {
        ...
        if (a.koef[i][COEF] != 0) {
            isZero = false;
            //Íå ïåðâûé è ïîëîæèòåëüíûé îäíî÷ëåí - âûâîäèì ïëþñ
            if (!firstPrint && a.koef[i][COEF]>0) {
                printf("+");
            }
            //Îäíî÷ëåí ñ íåíóëåâîé ñòåïåíüþ
            if (a.koef[i][DEG] != 0) {
                if (a.koef[i][COEF] != 1) {
                    //Ñëó÷àé: -1x
                    if (a.koef[i][COEF] == -1) {
...
int main(int argc, char*argv[]) {
	...
	yyparse();
	fclose(f);
    //ïðèíöèïû ÿçûêà, ãðàììàòèêà â îò÷åòå, ïî÷åìó ñòàò ìàññèâ?, 
    //ñòðóêòóðèðîâàòü îøèáêè ïî êàò, îøèáêà íóëåâîé ñòðîêè x^0 = y, êàê áîðîëèñü ñ îøèáêàìè yacc shift/reduce, 
}
